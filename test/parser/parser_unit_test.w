...
fn test_trait_layout_contains_assoc_and_methods:
    ...
    // Layout: [trait_tp_count, trait_tp_start, assoc_count, per-assoc
    // (name, bound_count, bounds..., default_type), method_count, ...]
    let assoc_count = pool.get_extra(extra_start + 2)
    assert(assoc_count == 1)
    let method_count_idx = extra_start + 2 + 1 + 1 + 1 + 1 + 1
