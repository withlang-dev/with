...
unsafe fn set_to_42(p: *mut i32):
    *p = 42

fn test_pointer_as_argument:
    var x: i32 = 0
    unsafe { set_to_42(&raw mut x) }
    assert(x == 42)

unsafe fn swap(a: *mut i32, b: *mut i32):
    let tmp = *a
    *a = *b
    *b = tmp
...
fn test_swap:
    var x: i32 = 10
    var y: i32 = 20
    unsafe { swap(&raw mut x, &raw mut y) }
    assert(x == 20)
    assert(y == 10)

unsafe fn add_to(p: *mut i32, val: i32):
    *p = *p + val

fn test_pointer_accumulate:
    var sum: i32 = 0
    unsafe { add_to(&raw mut sum, 10) }
    unsafe { add_to(&raw mut sum, 20) }
    unsafe { add_to(&raw mut sum, 30) }
    assert(sum == 60)
...
unsafe fn counter_get(c: *const Counter) -> i32:
    c.value
...
fn test_pointer_to_struct:
    ...
    unsafe { counter_inc(&raw mut c) }
    assert(unsafe { counter_get(&c) } == 2)

fn test_multiple_pointers:
    var a: i32 = 1
    var b: i32 = 2
    var c: i32 = 3
    unsafe { add_to(&raw mut a, 10) }
    unsafe { add_to(&raw mut b, 20) }
    unsafe { add_to(&raw mut c, 30) }
    assert(a == 11)
    assert(b == 22)
    assert(c == 33)
