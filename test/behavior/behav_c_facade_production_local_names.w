...
// D51 (spec §16.2b.4): each production form binds locals in its rendered
// constructor — the returned representation, the status, the in-place
// storage — and With refuses shadowing. A C parameter may be named like any
// of them, so the rendering spells its locals apart from the constructor's
// parameters: a direct-return producer taking `repr` and an in-place
// initializer taking `repr` and `status` render, and each argument reaches
// C as the caller wrote it.
