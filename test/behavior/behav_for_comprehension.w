...
fn test_option_none_first:
    let result: ?i32 = for x in get_opt(-1); y in double_positive(1):
        yield x + y
    assert(result == .None)

fn test_option_none_second:
    let result: ?i32 = for x in get_opt(5); y in double_positive(-1):
        yield x + y
    assert(result == .None)
...
fn test_result_err_propagation:
    let r: Result[i32, str] = for x in get_result(-1); y in get_result(1):
        yield x + y
    match r:
        Ok(_) => assert(false)
