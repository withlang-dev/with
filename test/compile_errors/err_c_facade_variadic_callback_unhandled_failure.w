...
// D66 (spec §16.2b.9): the pair's state includes setter failures. A
// userdata setter whose status is never compared against the operation's
// `ok` constant may have failed, leaving the callback paired with the
// foreign default, so the run is refused until the status is checked.
