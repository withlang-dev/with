...
fn main:
    var lexer1 = Lexer.init("fn let with match for if else return break continue goto async await unsafe type trait impl use module pub extend var comptime gen in as mut then loop while ephemeral spawn defer error extern enum", 0)
    ...
    assert(tokens1.get_tag(10) == TokenKind.TK_KW_GOTO())
    assert(tokens1.get_tag(tokens1.len() - 2) == TokenKind.TK_KW_ENUM())
