// Spec test: Section 4.3 — Record Update Syntax (formerly 25.21)

type Point { x: f64, y: f64 }

// PASS: basic update
fn test_basic_record_update:
    let p1 = Point { x: 1.0, y: 2.0 }
    let p2 = { p1 with x: 3.0 }
    assert(p2.x == 3.0)
    assert(p2.y == 2.0)

// PASS: multiple field update
fn test_multi_field_update:
    let p = Point { x: 1.0, y: 2.0 }
    let p2 = { p with x: 10.0, y: 20.0 }
    assert(p2.x == 10.0 and p2.y == 20.0)

// PASS: update non-Copy (moves base) — uses str which is non-Copy
// type Entity { name: str, hp: i32, pos: Point }
// fn test_update_non_copy:
//     let e = Entity { name: "hero", hp: 100, pos: Point { x: 0.0, y: 0.0 } }
//     let e2 = { e with hp: 90 }
//     assert(e2.hp == 90)
//     assert(e2.name == "hero")
