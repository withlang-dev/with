...
fn literal_code(slot: *mut *const i8, pattern: str, options: i32) -> *const i8:
    if slot as i64 == 0:
        return null
    ...
    let compiled = unsafe { with_regex_compile(pattern, options, &raw mut err_code, &raw mut err_offset) }
    if compiled as i64 == 0:
        unsafe { with_panic("invalid regex literal: " ++ with_regex_error_message(err_code), "", 0) }
        return null
