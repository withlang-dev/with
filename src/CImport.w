...
fn process_c_import(header_spec: str) -> str:
    ...
    while i < count:
        ...
        if kind == CK_FUNCTION:
            ...
            // Static asserts are informational — emit as comment
...
fn ci_compute_field_alignment(session: i64, idx: i32, fi: i32, field_count: i32) -> i64:
    ...
    if field_offset == 0:
        let head_align = ci_head_field_alignment(session, idx, field_count, parent_align)
        return head_align

    // Check remainder: offset % natural_align
    let remainder = field_offset % natural_align
    if remainder > 0:
        // Alignment is smaller than natural — find effective alignment from remainder
        if remainder > 0 and ci_is_power_of_two(remainder):
    ...
    let offset_align = field_offset  // offset itself is the offset alignment
    let possible = if parent_align < offset_align: parent_align else: offset_align
    if possible == natural_align:
        return 0  // naturally aligned
    if possible < natural_align:
    ...
    if fi > 0:
        ...
        if prev_offset >= 0 and prev_size >= 0:
            ...
            if padding < natural_align:
                return 0  // natural padding, no annotation needed
            return possible
    0

// Look ahead through fields to determine if the first field needs an alignment
// annotation. If struct pointer alignment differs from max field natural alignment,
// the first field needs explicit alignment.
...
fn ci_default_for_type(ty: str) -> str:
    ...
    // Struct/union types (known to c_import) → use default struct init
    // This handles nested struct fields getting zero-initialized
    ""
...
// Get the last non-whitespace character before position idx, or 0 if none.
...
fn ci_is_blank_macro_ref(value: str, blank_macros: str) -> bool:
    ...
    if trimmed.len() == 0:
        return true
    // Check each whitespace-separated token — all must be blank macros
    var pos = 0
    ...
    while pos < slen:
        ...
        while pos < slen:
            ...
            pos = pos + 1
        if pos == tok_start:
            return false  // Non-identifier token — not a blank macro ref
        let tok = trimmed.slice(tok_start as i64, pos as i64)
        if not ci_str_contains(blank_macros, "|" ++ tok ++ "|"):
            return false  // Token is not a known blank macro
        found_any = true
    found_any
