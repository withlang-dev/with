...
pub fn migrate_c_file(input_path: str, output_path: str) -> i32:
    ...
    // Import With standard library for memory and string operations
    // (no c_import — migrated code uses With's allocator, not libc)
    output = output ++ "use std.mem\nuse std.string\n\n"
...
fn ci_is_system_path(loc: str) -> bool:
    ...
    // Temp file from cimport_parse (the wrapper, not user code)
