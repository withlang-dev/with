...
pub fn migrate_c_file(input_path: str, output_path: str) -> i32:
    ...
    // Self-contained — no std imports. Runtime functions as extern fn.
    // Inline ctype/string helpers (no dependency on std.string).
    ...
    // String operations — link to libc (available via libSystem on macOS)
    ...
    if with_cimport_is_name_emitted("with_clz") == 0:
        ...
        output = output ++ "extern fn with_ctzll(x: i64) -> i32\n"
        output = output ++ "extern fn with_abs(x: i32) -> i32\n"
        // Memory and allocation — all use *i8 (cast at call site)
    ...
    // Filter: skip system declarations, keep user code.
    // Uses source location when available, falls back to name patterns.
    var i = 0
    while i < count:
        ...
        if decl_name.len() == 0:
            i = i + 1
            continue
        // Skip C reserved names (__ or _[A-Z])
        if decl_name.len() >= 2 and decl_name.byte_at(0) == 95:
            let second = decl_name.byte_at(1)
            if second == 95 or (second >= 65 and second <= 90):
                i = i + 1
                continue
        // Skip libc functions we map to With equivalents
        if ci_is_mapped_libc_fn(decl_name):
            i = i + 1
            continue
        let kind = with_cimport_decl_kind(session, i)
        if kind == CK_FUNCTION:
    ...
    let macro_include = "#include \"" ++ input_path ++ "\"\n"
...
// Check if a declaration is from a system header using source location.
// This filters out the noise from stdlib.h, string.h, ctype.h, etc.
// Check if a declaration is from a system header using source location.
// Correct by construction — no name blacklist needed.
fn ci_is_system_decl_by_location(session: i64, name: str) -> bool:
    if name.len() == 0: return true
    // C reserved names: __ prefix or _[A-Z] prefix
    if name.len() >= 2 and name.byte_at(0) == 95:
        let second = name.byte_at(1)
        if second == 95 or (second >= 65 and second <= 90):
            return true
    // Check source location — system headers live in /usr/, /SDKs/, etc.
    let loc = ci_get_decl_location(session, name)
    if loc.len() > 0:
        if ci_str_contains(loc, "/usr/"): return true
        if ci_str_contains(loc, "/SDKs/"): return true
        if ci_str_contains(loc, "/Applications/Xcode"): return true
        if ci_str_contains(loc, "/Library/"): return true
        if ci_str_contains(loc, "/clang/"): return true
        // Temp file created by cimport_parse (wrapper, not user code)
        if ci_str_contains(loc, "with_cimport_"): return true
        return false  // has location, not system → keep it
    // No location info (macros) — keep if name looks like user code
    // Skip libc functions we map to With equivalents
...
fn ci_is_mapped_libc_fn(name: str) -> bool:
    ...
    if name == "strstr" or name == "strcpy" or name == "strncpy" or name == "strcat" or name == "strncat": return true
    false
...
fn ci_map_libc_call(callee: str, args: str) -> str:
    ...
    if callee == "strncmp":
        return "string_cmp(" ++ args ++ ")"  // TODO: strncmp needs length param
    if callee == "strchr":
    ...
    // memchr — keep as extern (no With equivalent yet)
    // Already declared via c_import or as extern fn in the preamble
