...
fn Parser.parse_extern_decl(self: Parser, start: i32) -> i32:
    ...
    self.pool.add_fn_meta(extern_node, is_variadic, ret_type, extra_start, param_count, 0, 0)
    extern_node
...
fn Parser.parse_type_decl(self: Parser, is_pub: i32, start: i32) -> i32:
    ...
    if self.peek() == TK_L_BRACE():
        ...
        return self.pool.add_node(NK_TYPE_DECL(), start, self.prev_end(), name, extra_start, TDK_STRUCT())

    // Enum: starts with | or Identifier followed by |
    if self.peek() == TK_PIPE():
        let extra_start = self.parse_enum_variants()
        self.pool.add_extra(is_pub)
        return self.pool.add_node(NK_TYPE_DECL(), start, self.prev_end(), name, extra_start, TDK_ENUM())
    ...
    if self.peek() == TK_KW_FN() or self.peek() == TK_IDENT() or self.peek() == TK_AMPERSAND() or self.peek() == TK_L_PAREN():
        ...
        self.pool.add_extra(aliased)
        self.pool.add_extra(is_pub)
        return self.pool.add_node(NK_TYPE_DECL(), start, self.prev_end(), name, extra_start, TDK_ALIAS())
...
fn Parser.parse_error_decl(self: Parser, is_pub: i32, start: i32) -> i32:
    ...
    if self.is_ident_named("from"):
        ...
        self.pool.add_extra(is_pub)
        return self.pool.add_node(NK_TYPE_DECL(), start, self.prev_end(), err_name, extra_start, TDK_ENUM())
    ...
    let extra_start = self.parse_enum_variants()
    self.pool.add_extra(is_pub)
    self.pool.add_node(NK_TYPE_DECL(), start, self.prev_end(), err_name, extra_start, TDK_ENUM())
...
fn Parser.parse_impl_block(self: Parser, vis: i32):
    ...
    while self.peek() == TK_KW_FN() or self.peek() == TK_KW_PUB() or self.peek() == TK_KW_ASYNC() or self.peek() == TK_KW_TYPE():
        ...
        self.pool.add_fn_meta(fn_node, flags, ret_type, m_params_start, param_count, m_tp_start, m_tp_count)
        self.pool.add_decl(fn_node)
        method_count = method_count + 1
        self.skip_newlines()

    // Emit impl_decl node. Store method_count in extra so Codegen can walk backwards.
...
fn Parser.parse_type_expr(self: Parser) -> i32:
    ...
    if t == TK_STAR():
        ...
        if self.peek() == TK_KW_MUT():
            is_mut = 1
            self.advance()
        else if self.peek() == TK_IDENT() and self.is_ident_named("const"):
            self.advance()
        let pointee = self.parse_type_expr()
...
fn Parser.parse_optional_where_clause(self: Parser):
    if not self.is_ident_named("where"):
        return
    self.advance()
    while self.peek() == TK_IDENT():
        self.expect_ident()
        self.expect(TK_COLON())
        self.parse_type_bound_symbol()
        while self.peek() == TK_PLUS():
            self.advance()
            self.parse_type_bound_symbol()
        if self.peek() != TK_COMMA():
