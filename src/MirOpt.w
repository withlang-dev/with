...
fn devirtualize(mod: MirOptModule) -> i32:
    var changed = 0
    var fi = 0
    while fi < mod.functions.len() as i32:
        let func = mod.functions.get(fi as i64)
        var ci = 0
        while ci < func.calls.len() as i32:
...
fn promote_non_escaping_boxes(mod: MirOptModule) -> i32:
    ...
    while fi < mod.functions.len() as i32:
        let func = mod.functions.get(fi as i64)
        var ai = 0
        while ai < func.allocations.len() as i32:
...
fn eliminate_dead_fields(mod: MirOptModule) -> i32:
    var changed = 0
    var ti = 0
    while ti < mod.types.len() as i32:
        let ty = mod.types.get(ti as i64)
        var fi = 0
        while fi < ty.fields.len() as i32:
...
fn elide_redundant_moves(mod: MirOptModule) -> i32:
    ...
    while fi < mod.functions.len() as i32:
        let func = mod.functions.get(fi as i64)
        var mi = 0
        while mi < func.moves.len() as i32:
