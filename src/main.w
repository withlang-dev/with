...
fn build_graph_pcre2_add_imports(path: str, sentinel: str, insertion: str) -> i32:
    ...
    if with_str_contains(text, sentinel) != 0:
        return 0
    let updated = build_graph_insert_after_defs_import(text, insertion)
    if updated == text:
        return 0
    with_fs_write_file(path, updated)
...
fn build_graph_pcre2_ensure_generated_dependencies(generated_dir: str) -> i32:
    ...
    if build_graph_pcre2_add_imports(compile_path, "use std.re.pcre2_auto_possess", compile_imports) != 0:
        with_eprint("error: pcre2 generated check could not update imports in " ++ compile_path)
        return 1
    ...
    if build_graph_pcre2_add_imports(auto_path, "use std.re.pcre2_xclass", "use std.re.pcre2_xclass\n") != 0:
        with_eprint("error: pcre2 generated check could not update imports in " ++ auto_path)
        return 1

    let pcre2test_path = resolve_join(generated_dir, "pcre2test.w")
    if with_fs_file_exists(pcre2test_path) == 0:
        return 0
    let pcre2test_text = with_fs_read_file(pcre2test_path)
    if with_str_contains(pcre2test_text, "use std.re.pcre2_context") != 0:
        return 0
    ...
    if updated_pcre2test != pcre2test_text:
        if with_fs_write_file(pcre2test_path, updated_pcre2test) != 0:
            with_eprint("error: pcre2 generated check could not update imports in " ++ pcre2test_path)
            return 1
    0
...
fn build_graph_pcre2_line_starts_with_fn_main(line: str) -> bool:
    ...
    while j < line.len() as i32:
        ...
        if ch != 32 and ch != 9:
            break
        j = j + 1
    with_str_starts_with(line.slice(j as i64, line.len()), "fn main") != 0
...
fn build_graph_pcre2_module_body_for_synthetic_check(text: str) -> str:
    ...
    for i in 0..text.len() as i32:
        if text.byte_at(i as i64) == 10:
            let line = text.slice(line_start as i64, (i + 1) as i64)
            if line_no > 2 and with_str_starts_with(line, "use std.re.") == 0:
                out = out ++ line
            line_start = i + 1
            line_no = line_no + 1
    if line_start < text.len() as i32:
...
fn build_graph_count_error_lines(text: str) -> i32:
    ...
    for i in 0..text.len() as i32:
        if text.byte_at(i as i64) == 10:
            ...
            if with_str_contains(line, "error:") != 0:
                count = count + 1
            line_start = i + 1
    if line_start < text.len() as i32:
        let line = text.slice(line_start as i64, text.len())
...
fn build_graph_pcre2_count_generated_errors(root: str, target: BuildGraphTarget, compiler_path: str, generated_dir: str, print_summary: bool) -> i32:
    if with_fs_file_exists(compiler_path) == 0:
        with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' missing compiler: " ++ compiler_path)
        return -1
    if with_fs_is_dir(generated_dir) == 0:
        with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' missing generated directory: " ++ generated_dir)
        return -1
    let deps_rc = build_graph_pcre2_ensure_generated_dependencies(generated_dir)
    if deps_rc != 0:
        return -1
    let defs_path = resolve_join(generated_dir, "defs.w")
    if with_fs_file_exists(defs_path) == 0:
        with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' missing generated defs.w: " ++ defs_path)
        return -1
    let defs_text = with_fs_read_file(defs_path)
    let files = collect_test_files(generated_dir)
    let tmp_dir = resolve_join(resolve_join(root, "out/tmp"), "pcre2-generated-check")
    if with_fs_mkdir_p(tmp_dir) != 0:
        with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' could not create temp directory: " ++ tmp_dir)
        return -1
    let tmp_path = resolve_join(tmp_dir, f"{with_getpid()}.{with_clock_nanos()}.w")
    ...
    for fi in 0..files.len() as i32:
        ...
        if mod_name == "defs":
            continue
        let module_text = with_fs_read_file(path)
        ...
        if not build_graph_pcre2_module_defines_main(module_text):
            synthetic = synthetic ++ "\nfn main { print(\"ok\") }\n"
        if with_fs_write_file(tmp_path, synthetic) != 0:
            with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' could not write temp file: " ++ tmp_path)
            return -1
        var argv = ""
        argv = build_graph_argv_append(argv, compiler_path)
        argv = build_graph_argv_append(argv, "check")
        argv = build_graph_argv_append(argv, tmp_path)
        let rc = with_exec_argv_capture(argv, check_out, check_out, 180000)
        if rc == 124:
            with_eprint("error: pcre2 generated check target '" ++ target.name ++ "' timed out checking module: " ++ mod_name)
            return -1
        let output = with_fs_read_file(check_out)
        let errors = build_graph_count_error_lines(output)
        if errors == 0:
            ok = ok + 1
        else:
            with_write(mod_name ++ f" {errors} {module_text.len()}\n")
            total_errors = total_errors + errors
        let _remove_check_out = with_fs_remove_file(check_out)
    let _remove_tmp = with_fs_remove_file(tmp_path)
    if print_summary:
        with_write(f"OK={ok} TOTAL_ERRORS={total_errors}\n")
    total_errors

fn build_graph_run_pcre2_generated_check(root: str, target: BuildGraphTarget) -> i32:
    if target.entry.len() == 0 or target.inputs.len() == 0:
        with_eprint("error: pcre2_generated_check target '" ++ target.name ++ "' requires compiler entry and generated-dir input")
        return 1
    let arg_rc = build_graph_validate_process_args(target)
    if arg_rc != 0:
        return arg_rc
...
fn build_graph_run_pcre2_generated_promote(root: str, target: BuildGraphTarget) -> i32:
    if target.entry.len() == 0 or target.inputs.len() == 0 or target.output.len() == 0:
        with_eprint("error: pcre2_generated_promote target '" ++ target.name ++ "' requires compiler entry, generated-dir input, and destination output")
        return 1
    let arg_rc = build_graph_validate_process_args(target)
    if arg_rc != 0:
        return arg_rc
    ...
    if errors < 0:
        return 1
    if errors != 0:
        with_eprint(f"error: refusing to promote generated PCRE2 with {errors} remaining errors")
        return 1
    if with_fs_mkdir_p(dest_dir) != 0:
        with_eprint("error: pcre2_generated_promote target '" ++ target.name ++ "' could not create destination: " ++ dest_dir)
        return 1
    let existing = collect_test_files(dest_dir)
    for ei in 0..existing.len() as i32:
        let _remove_old = with_fs_remove_file(existing.get(ei as i64))
    let files = collect_test_files(generated_dir)
    var copied = 0
    for fi in 0..files.len() as i32:
        ...
        let contents = with_fs_read_file(source_path)
        if with_fs_write_file(dest_path, contents) != 0:
            with_eprint("error: pcre2_generated_promote target '" ++ target.name ++ "' could not copy " ++ source_path ++ " to " ++ dest_path)
            return 1
        copied = copied + 1
    with_write(f"promoted {copied} generated modules into {dest_dir}\n")
    0

fn build_graph_copy_w_files(source_dir: str, dest_dir: str) -> i32:
    let files = collect_test_files(source_dir)
    if files.len() == 0:
        with_eprint("error: no .w files found in " ++ source_dir)
        return 1
    if with_fs_mkdir_p(dest_dir) != 0:
        with_eprint("error: could not create destination directory: " ++ dest_dir)
        return 1
    for fi in 0..files.len() as i32:
        let source_path = files.get(fi as i64)
        let dest_path = resolve_join(dest_dir, build_graph_path_basename(source_path))
        let contents = with_fs_read_file(source_path)
        if with_fs_write_file(dest_path, contents) != 0:
            with_eprint("error: could not copy " ++ source_path ++ " to " ++ dest_path)
            return 1
    0

fn build_graph_run_pcre2_build(root: str, target: BuildGraphTarget) -> i32:
    if target.entry.len() == 0 or target.inputs.len() == 0 or target.output.len() == 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' requires compiler entry, migrated-dir input, and output directory")
        return 1
    let arg_rc = build_graph_validate_process_args(target)
    if arg_rc != 0:
        return arg_rc
    ...
    let output_dir = build_graph_resolve_project_path(root, target.output)
    if with_fs_file_exists(compiler_path) == 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' missing compiler: " ++ compiler_path)
        return 1
    if with_fs_is_dir(migrated_dir) == 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' missing migrated PCRE2 directory: " ++ migrated_dir ++ " — run regex-migrate deliberately")
        return 1
    let tmp_dir = output_dir ++ ".tmp." ++ f"{with_getpid()}.{with_clock_nanos()}"
    let re_dir = resolve_join(resolve_join(resolve_join(tmp_dir, "lib"), "std"), "re")
    let bin_dir = resolve_join(tmp_dir, "bin")
    let _remove_tmp = with_fs_remove_dir(tmp_dir)
    if with_fs_mkdir_p(re_dir) != 0 or with_fs_mkdir_p(bin_dir) != 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' could not create temp build directories under " ++ tmp_dir)
        return 1
    ...
    if errors < 0:
        return 1
    if errors != 0:
        with_eprint(f"error: pcre2_build target '{target.name}' generated sources have {errors} remaining errors")
        return 1
    let pcre2test_src = resolve_join(re_dir, "pcre2test.w")
    let pcre2test_bin = resolve_join(bin_dir, "pcre2test")
    if with_fs_file_exists(pcre2test_src) == 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' missing pcre2test source after copy: " ++ pcre2test_src)
        return 1
    ...
    if rc == 124:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' timed out building pcre2test; stdout=" ++ stdout_path ++ " stderr=" ++ stderr_path)
        return 124
    if rc != 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ f"' failed building pcre2test with exit code {rc}; stdout=" ++ stdout_path ++ " stderr=" ++ stderr_path)
        return if rc == 0: 1 else: rc
    if with_fs_file_exists(pcre2test_bin) == 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' did not produce pcre2test binary: " ++ pcre2test_bin)
        return 1
    let _remove_old = with_fs_remove_dir(output_dir)
    var mv_argv = ""
    ...
    if with_exec_argv(mv_argv) != 0:
        with_eprint("error: pcre2_build target '" ++ target.name ++ "' could not move temp tree to " ++ output_dir)
        return 1
    with_write("built migrated PCRE2: " ++ resolve_join(output_dir, "bin/pcre2test") ++ "\n")
    0
