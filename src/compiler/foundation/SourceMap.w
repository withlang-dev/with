...
pub fn SourceMap.add_source_text(self: SourceMap, path: str, text: str) -> FileId:
...
pub fn SourceMap.add_source_file(self: SourceMap, path: str) -> FileId:
    let existing = self.path_index.get(path)
    if existing.is_some():
        return file_id_from_raw(existing.unwrap())
...
pub fn SourceMap.contains(self: SourceMap, file_id: FileId) -> bool:
    if not file_id_is_valid(file_id):
        return false
    let raw = file_id_raw(file_id)
    raw >= 0 and raw < self.sources.len() as i32

pub fn SourceMap.get_source(self: SourceMap, file_id: FileId) -> Source:
    if not self.contains(file_id):
        return self.sources.get(0)
    self.sources.get(file_id_raw(file_id) as i64)

pub fn SourceMap.offset_to_location(self: SourceMap, file_id: FileId, offset: i32) -> SourceLocation:
    if not self.contains(file_id):
        return SourceLocation { line: 0, col: 0 }
    self.get_source(file_id).offset_to_location(offset)

pub fn SourceMap.line_text(self: SourceMap, file_id: FileId, line: i32) -> str:
    if not self.contains(file_id):
        return ""
