...
pub fn lock_write(project_root: &str, lock: &LockFile) -> i32:
    ...
    for i in 0..lock.entries.len() as i32:
        ...
        text = text ++ lock_json_string("version", entry.version, entry.source == "conan" or entry.source == "port")
        if entry.source == "port": text = text ++ lock_json_string("sha256", entry.sha256, false)
...
fn lock_entry_from_installed_c_dep(project_root: &str, name: &str, version: &str) -> LockEntry:
    ...
    // Built from a port: what pins it is the source tarball's digest.
    if recipe_rev == "port":
        return LockEntry { name: dep_name, source: "port", version: with_str_clone_ref(version), recipe_rev: "", package_id: "", package_rev: "", sha256: package_rev }
