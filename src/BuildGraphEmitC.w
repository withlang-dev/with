...
fn emitc_collect_bridge_exports(root: str, target_name: str) -> Vec[EmitCFunction]:
    ...
    let sources: [2]str = ["rt/llvm_bridge.w", "rt/clang_bridge.w"]
    for si in 0..2:
...
fn emitc_collect_runtime_exports(root: str, target_name: str) -> Vec[EmitCFunction]:
    let path = build_graph_resolve_project_path(root, "rt/regex_runtime.w")
    let text = build_graph_rt_read_file(path)
    if text.len() == 0:
        build_graph_rt_eprint("error: emit_c_test target '" ++ target_name ++ "' could not read runtime source: " ++ path)
        return Vec.new()
    emitc_collect_exports_from_text(text, path, target_name)
...
fn emitc_generate_stub_files(root: str, target_name: str) -> i32:
    let stub_exports = emitc_collect_bridge_exports(root, target_name)
    ...
    for ei in 0..stub_exports.len() as i32:
        ...
        stubs = stubs ++ "}\n\n"
    let runtime_exports = emitc_collect_runtime_exports(root, target_name)
    if runtime_exports.len() == 0:
        build_graph_rt_eprint("error: emit_c_test target '" ++ target_name ++ "' found no runtime exports")
        return 1
