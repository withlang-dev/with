...
fn col_of(text: &str, offset: i32):
    var j = offset - 1
    while j >= 0 and (text[j] as i32) != 10:
        j = j - 1
    offset - (j + 1)
...
fn trim(s: &str):
    let m = s.len() as i32
    var a = 0
    while a < m and (s[a] as i32) == 32:
        a = a + 1
    var b = m
    while b > a and (s[(b - 1)] as i32) == 32:
        b = b - 1
    slice(s, a, b)
...
fn tparam_names(inner: &str):
    ...
    while i <= m:
        let at_end = i == m
        let c = if at_end: 44 else: inner[i] as i32
        if c == 91:
            depth = depth + 1
        else if c == 93:
            depth = depth - 1
            ...
            var colon = seg_start
            while colon < i and (inner[colon] as i32) != 58:
                colon = colon + 1
...
fn relocation_fact_at(facts: &RelocationFacts, path: &str, offset: i32) -> i32:
    for i in 0..facts.starts.len() as i32:
        if facts.paths[i] == path and offset >= facts.starts[i] and offset < facts.ends[i]: return i
    -1
...
impl RelocationFacts:
    mut fn relocate_file(path: &str, apply: bool, list_methods: bool) -> i32:
        ...
        while i < n:
            ...
            let semantic_index = relocation_fact_at(self, path, fn_pos)
            if semantic_index < 0:
                skipped = skipped + 1
                open_header = ""
                i = i + 1
                continue
            if self.matched[semantic_index] != 0:
                print(f"error: {path}: duplicate relocation match at byte {fn_pos}")
                return -1
            self.matched[semantic_index] = 1
            let mode = self.modes[semantic_index]
            if source_mode != mode:
            ...
            while rti < rlen and (recv_type[rti] as i32) == 38:   // &
                rti = rti + 1
            while rti < rlen and (recv_type[rti] as i32) == 32:   // space
                rti = rti + 1
            ...
            var bpos = rti
            while bpos < rlen and (recv_type[bpos] as i32) != 91:   // '['
                bpos = bpos + 1
            if bpos < rlen:
            ...
            if q < n and tokens.get_tag(q) == TokenKind.TK_COMMA:
                ...
                while self_del_end < tlen and (text[self_del_end] as i32) == 32:
            ...
            while body_end < tlen and (text[body_end] as i32) != 10:
                body_end = body_end + 1
            if body_end < tlen:
                body_end = body_end + 1
            ...
            if has_tp:
                ...
                var ta = 0
                while ta < tl and (target[ta] as i32) != 91:  // find [
                    ta = ta + 1
                if ta >= tl:
                    ok = false
                else:
                    var tb = tl - 1
                    while tb > ta and (target[tb] as i32) != 93:  // find ]
                        tb = tb - 1
        ...
        for fi in 0..self.matched.len() as i32:
            if self.paths[fi] == path and self.matched[fi] == 0:
                print(f"error: {path}: compiler-selected declaration at byte {self.starts[fi]} was not structurally matched")
                return -1
...
fn verify_all_matched(facts: &RelocationFacts, excludes: &Vec[str]) -> bool:
    for i in 0..facts.matched.len() as i32:
        ...
        if facts.matched[i] == 0:
            print(f"error: {facts.paths[i]}: compiler-selected declaration at byte {facts.starts[i]} was not processed")
            return false
    true

impl RelocationFacts:
    mut fn reset_matches():
        for i in 0..self.matched.len() as i32: self.matched[i] = 0
...
pub fn run_receiver_migration -> i32:
    ...
    while arg < argv.len() as i32:
        if argv[arg] == "--apply":
            apply = true
        else if argv[arg] == "--report":
            apply = false
        else if argv[arg] == "--list":
            list_methods = true
        else if argv[arg] == "--exclude":
            if arg + 1 >= argv.len() as i32:
                print("error: --exclude requires a file path")
                exit_code(1)
            arg = arg + 1
            excludes.push(with_str_clone_ref(argv[arg]))
        else:
            if entry.len() > 0:
                print("error: exactly one semantic entry file is required")
                exit_code(1)
            entry = with_str_clone_ref(argv[arg])
        arg = arg + 1
    if entry.len() == 0:
        print("error: no semantic entry file supplied")
        exit_code(1)
    var facts = compiler_relocation_facts(entry)
