...
1. **Measure the gate as a RATIO, not an absolute number — this is the only thing left before landing.** Every mechanism is done; per-test arithmetic (`test` 0.40 s vs the seed's 0.39–0.54) puts the lane at or under the seed's own 144 s. Eric's ruling (2026-09-09): activity on his laptop blocks nothing — do **not** wait for an idle box. The gate is *relative* (≤ +10% over the pre-C4 baseline), so measure it under identical conditions where load cancels: (a) **CI, same runner class** — push the landing tree as `wo-c4`; its macOS `behavior-tests` step runs isolated; divide by main's step time on the same runner (the last green main runs, `79d523f4`/`33add964`, are the baseline). Ratio ≤ 1.10 is the gate. (b) Corroborate locally back-to-back: pre-C4 seed lane vs C4 lane, interleaved A/B/A/B under whatever load is present. A wall-clock number taken alone on the shared laptop (177 s, 282 s) is load noise and proves nothing either way.
