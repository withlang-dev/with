...
fn main -> i32:
    ...
    unsafe {
        ...
        i31_to_monty(&raw mut xv[0] as *mut u32, &m[0] as *const u32)
        i31_montmul(&raw mut t1[0] as *mut u32, &xv[0] as *const u32, &xv[0] as *const u32, &m[0] as *const u32, m0i)
        i31_from_monty(&raw mut t1[0] as *mut u32, &m[0] as *const u32, m0i)
        i31_encode(&raw mut got[0] as *mut u8, 77, &t1[0] as *const u32)
    }
    var bad = 0
    var i = 0
    while i < 77:
        let exp: u8 = if i == 76: 4u8 else: 0u8
        if got[i] != exp:
            bad = bad + 1
        i = i + 1
    if bad == 0:
