...
fn main -> i32:
    var m_bytes: [u8; 32] = [0u8; 32]
    var x_bytes: [u8; 32] = [0u8; 32]
    var e_bytes: [u8; 3] = [0u8; 3]
    var want: [u8; 32] = [0u8; 32]
    m_bytes[0] = 128u8
    m_bytes[1] = 0u8
    m_bytes[2] = 0u8
    m_bytes[3] = 0u8
    ...
    var got: [u8; 32] = [0u8; 32]
    unsafe {
        i31_decode(&raw mut m[0] as *mut u32, &m_bytes[0] as *const u8, 32)
        let m0i = i31_ninv31(m[1])
        i31_decode_reduce(&raw mut xv[0] as *mut u32, &x_bytes[0] as *const u8, 32, &m[0] as *const u32)
        ...
        i31_encode(&raw mut got[0] as *mut u8, 32, &xv[0] as *const u32)
    }
    var bad = 0
    var i = 0
    while i < 32:
