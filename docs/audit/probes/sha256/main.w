...
fn main:
    var abc: [u8; 3] = [97 as u8, 98 as u8, 99 as u8]
    var ax: [u8; 64] = [97 as u8; 64]
    var digest: [u8; 32] = [0 as u8; 32]
    sha256_hash(&abc[0] as *const u8, 0, &raw mut digest[0] as *mut u8)
    print("V0 " ++ sha256_hex(&digest[0] as *const u8))
    sha256_hash(&ax[0] as *const u8, 1, &raw mut digest[0] as *mut u8)
    print("V1 " ++ sha256_hex(&digest[0] as *const u8))
    sha256_hash(&abc[0] as *const u8, 3, &raw mut digest[0] as *mut u8)
    print("V3 " ++ sha256_hex(&digest[0] as *const u8))
    sha256_hash(&ax[0] as *const u8, 55, &raw mut digest[0] as *mut u8)
    print("V55 " ++ sha256_hex(&digest[0] as *const u8))
    sha256_hash(&ax[0] as *const u8, 56, &raw mut digest[0] as *mut u8)
    print("V56 " ++ sha256_hex(&digest[0] as *const u8))
    sha256_hash(&ax[0] as *const u8, 64, &raw mut digest[0] as *mut u8)
    print("V64 " ++ sha256_hex(&digest[0] as *const u8))
    var ctx = Sha256.new()
    let p = &raw mut ctx as *mut Sha256
    unsafe { sha256_update(p, &abc[0] as *const u8, 1) }
    unsafe { sha256_update(p, &abc[1] as *const u8, 2) }
    unsafe { sha256_finish(p, &raw mut digest[0] as *mut u8) }
    print("VSPLIT " ++ sha256_hex(&digest[0] as *const u8))
