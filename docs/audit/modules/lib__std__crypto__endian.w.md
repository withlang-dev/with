# Audit: lib/std/crypto/endian.w @ 450733e58a1a7cce14f9cb2084943fc178815111 — COMPLETE

Module: byte-order encode/decode helpers for crypto/network code
(endian.w:1-3; intended as future compiler intrinsics `u32.from_be` etc.).
12 functions, ALL module-private (no `pub`): `u16/u32/u64 × from_be/from_le/
to_be/to_le` (endian.w:5-65). Sole in-repo consumer path: `std.crypto.sha256`
(`u32_from_be` compress, `u64_to_be` length, `u32_to_be` digest).

## Scope examined
- Full file read (65 lines @ 450733e58a1a7cce14f9cb2084943fc178815111).
...
- O1 `python3 struct.unpack` (`>H/<H/>I/<I/>Q/<Q`), an independent
  implementation of positional byte order, for all six decode directions:
```
u32_from_be struct=0x12345678 identity=0x12345678 MATCH
u32_from_le struct=0x78563412 identity=0x78563412 MATCH
u16_from_be struct=0x1234 identity=0x1234 MATCH
u16_from_le struct=0x3412 identity=0x3412 MATCH
u64_from_be struct=0x102030405060708 identity=0x102030405060708 MATCH
u64_from_le struct=0x807060504030201 identity=0x807060504030201 MATCH
```
- O2 Byte-order identities (definitional): `from_X(to_X(v)) == v`,
  `from_be != from_le` on asymmetric input, nonzero-offset loads, off-6
  stores — executed in P1.
- O3 Transitive reachable-path proof: sha256 P1 vectors execute
  `u32_from_be` (every block), `u64_to_be` (every finish), `u32_to_be`
  (every digest) on real data — all digests MATCH hashlib.

## Probes
- P1 `docs/audit/probes/endian/main.w` — vendored copy of HEAD source text
  (endian.w verbatim, file has no `use` lines) + `fn main` with 14 checks
  (6 decode vectors, 6 roundtrips incl. `0xDEADBEEF`/`0xABCD`/
  `0x0123456789ABCDEF`, 2 nonzero-offset loads):
  EXECUTED (endian_exit=0).
  Command: `out/bootstrap/bin/with-stage1 run docs/audit/probes/endian/main.w`.
  (Vendoring is the only external path: symbols are module-private, so a
  standalone probe cannot `use` them; the body tested IS the audited text.)
```
PASS u32_from_be
PASS u32_from_le
PASS u16_from_be
...
PASS u64_from_be
PASS u64_from_le
PASS u32 be roundtrip
PASS u32 le roundtrip
PASS u64 be roundtrip
PASS u64 le roundtrip
PASS u16 be roundtrip off6
PASS u16 le roundtrip off6
PASS u32_from_be off1
PASS u16_from_le off2
```
  (14 PASS, 0 FAIL.)
- P2 Reachable-path via sha256: EXECUTED — every sha256 P1 vector runs three
  endian fns; all 7 MATCH (see sha256 report). Covers `u32_from_be`,
  `u64_to_be`, `u32_to_be` end-to-end; the remaining 9 fns are covered by P1
  roundtrips/vectors.

## Traces
- T13 ownership/drop: clean. Pure register/stack transforms over caller
  buffers; no allocation, no Drop, no aliasing (each store writes disjoint
  bytes; each load reads its own window).
- T15 migration fidelity: never-`pub` surface is longstanding intent
  (helpers for std-internal crypto), not a regression; callers use them
  intra-std (sha256.w:4 `use std.crypto.endian`).
- T22 conformance (positional byte order, line-by-line): `u16_from_be`
  `(b0<<8)|b1` (endian.w:8), `u16_from_le` `b0|(b1<<8)` (endian.w:13),
  `u32_from_be` 24/16/8/0 (endian.w:20), `u32_from_le` 0/8/16/24 (endian.w:27),
  `u64_from_be` hi=`from_be(off)` lo=`from_be(off+4)` (endian.w:30-32),
  `u64_from_le` lo=`from_le(off)` hi=`from_le(off+4)` (endian.w:34-37), stores
  mirror loads byte-for-byte (endian.w:39-65); u64 stores compose u32 stores
  with correct half order (BE: hi first, endian.w:59-61; LE: lo first,
  endian.w:63-65). No overflow: widest shift is `b3<<24` on u32, `hi<<32` on
  u64 — exact fits. u16 operands are pre-widened (`as u16` from u8) before
  shifting, so `0xFF<<8` cannot overflow.

## Findings
- C1 `u64_from_le` half swap (lo/hi reversed): REFUTED — P1 `u64_from_le`
  vector MATCHes O1 struct `<Q`; roundtrip passes.
- C2 `u64_to_le` half order (lo-first vs hi-first): REFUTED — P1 le roundtrip
  on `0x0123456789ABCDEF` (halves differ) passes; BE/LE roundtrips would
  disagree on swapped halves.
- C3 Off-by-window loads (`off+1..off+3`, nonzero `off`): REFUTED — off1/off2
  and off-6 store/load checks pass.
- C4 Shift-width overflow (`<<24` on u32, `<<8` on u16): REFUTED by
  construction + execution (`0xDEADBEEF`/`0xABCD` roundtrips exact).

## Observations (non-defect)
- Obs-1: All-private surface means user programs cannot name these; the only
  external test paths are vendored-copy (P1) and transitive sha256 execution
  (P2) — both recorded honestly above.
- Obs-2: Endianness helpers duplicate per width instead of genericizing; fine
  as-is for 12 small fns, no change recommended.

Verdict: COMPLETE — all 14 checks pass, struct oracle agrees on all 6 decode directions, and sha256 transitively proves 3 fns on real data.
