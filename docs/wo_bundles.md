...
What the object defines (`Codegen.decl_path_is_bundle_owned`,
`decl_path_is_bundle_carried`): every function and global of a module
under `--bundle-corpus`, however it was reached; and, as *carried copies*
with internal linkage, the functions of any other With module the corpus
reaches that no other bundle provides — zlib's gz modules call std.libc's
`open`/`read`/`write`/`close`/`lseek` wrappers, which a consumer compiles
whole-program under bare names, so a declared-only module-link-named
reference from the object could never link; internal linkage lets each
bundle carry its own copy and `globaldce` drop the unreached ones. A
module another bundle provides stays declared-only. A carried module's
