...
| `src/main.w` | Cleanup and `run`/one-liner binary execution now use typed runtime wrappers. Remaining `with_system` uses are test stdout/stderr redirection and benchmark command execution. | Replace test/bench execution/redirection with argv process APIs that support env, cwd, capture, and timeout. |
