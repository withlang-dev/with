  ...
  if [[ "$stage0_rc" -ne 0 || "$self_rc_1" -ne 0 ]]; then
    if [[ "$stage0_rc" -ne "$self_rc_1" ]]; then
      if [[ -n "$kd_line" ]]; then
        ...
        echo "FAIL(wave5-typed-parity-status-mismatch) $src stage0=$stage0_rc selfhost=$self_rc_1"
...
if [[ "$declared_known_divergences" -ne "$used_known_divergences" ]]; then
  echo "FAIL(wave5-typed-parity-known-divergence-accounting) declared=$declared_known_divergences used=$used_known_divergences"
  failures=$((failures + 1))
fi

echo ""
echo "wave5 typed parity: processed=$processed failures=$failures known_divergences=$known_divergences"

if [[ "$failures" -ne 0 ]]; then
  echo "wave5 typed parity: FAIL"
  exit 1
fi
