...
- File I/O via `extern fn with_fs_read_file(path: str) -> str` /
  `with_fs_write_file(path: str, data: str) -> i32`. (Transitional: these decls
  are the D30-deprecated internal seam and die with it — post-retirement, tools
  reach fs through `std.fs`, and `str` in an extern signature is a §16.3e error.)
