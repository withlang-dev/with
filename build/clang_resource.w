...
fn cr_generate(ctx: ActionCtx, include_dir: str, files: Vec[str], version: str) -> str:
    ...
    out.push_str("// Auto-generated by with build embedded-clang-resource-source.\n")
    ...
    for i in 0..files.len() as i32:
        ...
        out.push_str(sym)
        out.push_str(": str = ")
        out.push_str(cr_raw_string_literal(source))
        out.push_str("\n")
        if listing.len() > 0:
            listing.push_str("\n")
        listing.push_str(rel)
    out.push_str("let CLANG_RES_LIST: str = ")
    out.push_str(cr_raw_string_literal(listing.to_str()))
    out.push_str("\n")
    out.push_str("let CLANG_RES_VERSION: str = ")
    out.push_str(cr_raw_string_literal(version))
    out.push_str("\n\n")
    out.push_str("pub fn embedded_clang_resource_list() -> str:\n    return CLANG_RES_LIST\n\n")
    out.push_str("pub fn embedded_clang_resource_version() -> str:\n    return CLANG_RES_VERSION\n\n")
    out.push_str("pub fn embedded_clang_resource_data(name: str) -> str:\n")
    for i in 0..files.len() as i32:
        let rel = cr_relpath(files.get(i as i64), include_dir)
        let sym = f"CLANG_RES_{i}"
        out.push_str("    if name == ")
        out.push_str(cr_raw_string_literal(rel))
        out.push_str(":\n")
        out.push_str("        return ")
        out.push_str(sym)
        out.push_str("\n")
