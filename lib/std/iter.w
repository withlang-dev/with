...
extern fn with_vec_len(v: *mut u8) -> i64
extern fn with_vec_get_i32(v: *mut u8, index: i64) -> i32
extern fn with_vec_get_str(v: *mut u8, index: i64) -> str
...
pub fn sum(arr: Vec[i32]) -> i32:
    ...
    while i < n:
        total = total + with_vec_get_i32((&raw mut arr) as *mut u8, i)
        i = i + 1
    total
...
pub fn map(arr: Vec[str], f: fn(str) -> i32) -> Vec[i32]:
    ...
    let n = with_vec_len((&raw mut arr) as *mut u8)
    while i < n:
        let x = with_vec_get_str((&raw mut arr) as *mut u8, i)
        let y = f(x)
        with_vec_push_i32((&raw mut result) as *mut u8, y)
        i = i + 1
    result
...
pub fn filter(arr: Vec[i32], pred: fn(i32) -> bool) -> Vec[i32]:
    ...
    let n = with_vec_len((&raw mut arr) as *mut u8)
    while i < n:
        let x = with_vec_get_i32((&raw mut arr) as *mut u8, i)
        if pred(x):
            with_vec_push_i32((&raw mut result) as *mut u8, x)
        i = i + 1
    result
