...
unsafe fn ecdsa_p256_verify(
    ...
    p256_load_n(&mut n[0] as *mut u32)
    ...
    i31_decode_reduce(&mut r[0] as *mut u32, sig_r, 32, np)
    i31_decode_reduce(&mut s[0] as *mut u32, sig_s, 32, np)
    ...
    fe_copy(&mut w[0] as *mut u32, &s[0] as *const u32)
    var wt1: [u32; 12] = [0u32; 12]
    var wt2: [u32; 12] = [0u32; 12]
    i31_modpow(&mut w[0] as *mut u32, &nm2[0] as *const u8, 32, np, n0i, &mut wt1[0] as *mut u32, &mut wt2[0] as *mut u32)

    // u1 = hash * w mod n
    var z: [u32; 12] = [0u32; 12]
    i31_decode_reduce(&mut z[0] as *mut u32, hash, 32, np)
    ...
    fe_copy(&mut z_m[0] as *mut u32, &z[0] as *const u32)
    fe_copy(&mut w_m[0] as *mut u32, &w[0] as *const u32)
    i31_to_monty(&mut z_m[0] as *mut u32, np)
    i31_to_monty(&mut w_m[0] as *mut u32, np)
    i31_montmul(&mut u1[0] as *mut u32, &z_m[0] as *const u32, &w_m[0] as *const u32, np, n0i)
    i31_from_monty(&mut u1[0] as *mut u32, np, n0i)

    // u2 = r * w mod n
    var u2: [u32; 12] = [0u32; 12]
    var r_m: [u32; 12] = [0u32; 12]
    fe_copy(&mut r_m[0] as *mut u32, &r[0] as *const u32)
    i31_to_monty(&mut r_m[0] as *mut u32, np)
    i31_montmul(&mut u2[0] as *mut u32, &r_m[0] as *const u32, &w_m[0] as *const u32, np, n0i)
    i31_from_monty(&mut u2[0] as *mut u32, np, n0i)

    // Encode u1, u2 back to bytes for point_mul
    var u1_bytes: [u8; 32] = [0u8; 32]
    var u2_bytes: [u8; 32] = [0u8; 32]
    i31_encode(&mut u1_bytes[0] as *mut u8, 32, &u1[0] as *const u32)
    i31_encode(&mut u2_bytes[0] as *mut u8, 32, &u2[0] as *const u32)
    ...
    fe_copy(&mut g_pt[0] as *mut u32, &gx[0] as *const u32)
    fe_copy(&mut g_pt[12] as *mut u32, &gy[0] as *const u32)
    fe_copy(&mut g_pt[24] as *mut u32, &gz[0] as *const u32)

    // Load public key Q in Jacobian Montgomery form
    var qx: [u32; 12] = [0u32; 12]
    var qy: [u32; 12] = [0u32; 12]
    var qz: [u32; 12] = [0u32; 12]
    i31_decode_reduce(&mut qx[0] as *mut u32, pub_x, 32, pp)
    i31_decode_reduce(&mut qy[0] as *mut u32, pub_y, 32, pp)
    ...
    fe_copy(&mut q_pt[0] as *mut u32, &qx[0] as *const u32)
    fe_copy(&mut q_pt[12] as *mut u32, &qy[0] as *const u32)
    fe_copy(&mut q_pt[24] as *mut u32, &qz[0] as *const u32)

    // Compute R = u1*G + u2*Q
    var r1_pt: [u32; 36] = [0u32; 36]
    var r2_pt: [u32; 36] = [0u32; 36]
    var result_pt: [u32; 36] = [0u32; 36]
    point_mul(&mut r1_pt[0] as *mut u32, &u1_bytes[0] as *const u8, &g_pt[0] as *const u32, pp, p0i)
    point_mul(&mut r2_pt[0] as *mut u32, &u2_bytes[0] as *const u8, &q_pt[0] as *const u32, pp, p0i)
    point_add(&mut result_pt[0] as *mut u32, &r1_pt[0] as *const u32, &r2_pt[0] as *const u32, pp, p0i)
    ...
    point_to_affine(&mut rx_bytes[0] as *mut u8, &mut ry_bytes[0] as *mut u8, &result_pt[0] as *const u32, pp, p0i)

    // v = R.x mod n
    var v: [u32; 12] = [0u32; 12]
    i31_decode_reduce(&mut v[0] as *mut u32, &rx_bytes[0] as *const u8, 32, np)

    // Check v == r
    // Compare by encoding both and comparing bytes
    var v_bytes: [u8; 32] = [0u8; 32]
    var r_bytes: [u8; 32] = [0u8; 32]
    i31_encode(&mut v_bytes[0] as *mut u8, 32, &v[0] as *const u32)
    i31_encode(&mut r_bytes[0] as *mut u8, 32, &r[0] as *const u32)
