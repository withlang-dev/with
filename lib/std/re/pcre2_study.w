        ...
        match __pc
            0 =>
                ...
                while true:
                    ...
                    var cs: *const u8 = null // init: untranslatable
                    var ce: *const u8 = null // init: untranslatable
                        ...
                        OP_DNREF =>
                            ...
                            if (if (if recno <= backref_cache[0]: 1 else: 0) != 0 and (if backref_cache[recno] >= 0: 1 else: 0) != 0: 1 else: 0) != 0:
                                ...
                                if (if ((re.overall_options & 512)) == 0: 1 else: 0) != 0:
                                    (ce = (cs = _pcre2_find_bracket_8(startcode, utf, recno)))
                                    if (if cs == null: 1 else: 0) != 0:
                                        return (0 - 2)

                                    if (if (not dupcapused) != 0 or (if _pcre2_find_bracket_8(ce, utf, recno) == null: 1 else: 0) != 0: 1 else: 0) != 0:
                                        if (if (if cc > cs: 1 else: 0) != 0 and (if cc < ce: 1 else: 0) != 0: 1 else: 0) != 0:
                                            (had_recurse = 1)
                                        else:
                                            var r: *mut recurse_check = null // init: untranslatable
                                            (r = recurses)
                                            while (if r != null: 1 else: 0) != 0:
if (if r.group == cs: 1 else: 0) != 0:
                                                    break
                                                (r = r.prev)

                                            if (if r != null: 1 else: 0) != 0:
                                                (had_recurse = 1)
                                            else:
...
(backref_cache[i] = (0 - 1))                                    (i = i + 1)

                                (backref_cache[0] = recno)

                            cc = cc + _pcre2_OP_lengths_8[unsafe: *cc]
                            // label: REPEAT_BACK_REFERENCE
match unsafe: *cc
                                OP_CRSTAR =>
                                    (cc = cc + 1)
                                OP_CRPLUS =>
...
if (if r.group == cs: 1 else: 0) != 0:
                                    ...
                                    if (if r != null: 1 else: 0) != 0:
                                        ...
                                        (this_recurse.prev = recurses)
                                        (this_recurse.group = cs)
                                        (prev_recurse_d = find_minlength(re, cs, startcode, utf, &this_recurse, countptr, backref_cache))
...
fn set_table_bit(re: *mut pcre2_real_code_8, p: *const u8, caseless: c_int, utf: c_int, ucp: c_int) -> *const u8:
    var p = p
...
re.start_bitmap[c] = re.start_bitmap[c] | re.tables[((c +% 512) +% cbit_type)]        (c = c + 1)


fn set_nottype_bits(re: *mut pcre2_real_code_8, cbit_type: c_int, table_limit: c_uint):
    var c: c_uint = 0 // init: untranslatable

fn set_start_bits(re: *mut pcre2_real_code_8, code: *const u8, utf: c_int, ucp: c_int, depthptr: *mut c_int) -> c_int:
    var code = code
    var c: c_uint = 0 // init: untranslatable
    ...
    while true:
        var try_next: c_int = 0 // init: untranslatable
        var tcode: *const u8 = null // init: untranslatable
        ...
        while try_next != 0:
            var rc: c_int = 0
            var ncode: *const u8 = null // init: untranslatable
            var classmap: *const u8 = null // init: untranslatable
            match unsafe: *tcode
                _ =>
                    return SSB_UNKNOWN
                OP_ACCEPT =>
                ...
                OP_PROP =>
                    if (if tcode[1] != 9: 1 else: 0) != 0:
                        return SSB_FAIL
                                        ...
                                        var p: *const c_uint = null // init: untranslatable

                    (try_next = 0)
                OP_WORD_BOUNDARY => 0
                OP_ASSERT =>
                    ncode = ncode + (1 + 2)
                    var done: c_int = 0 // init failed
                    while (not done) != 0:
                        match unsafe: *ncode
                            OP_ASSERT =>
                                ncode = ncode + (1 + 2)
                ...
                OP_TYPESTAR =>
                    tcode = tcode + 2
                OP_NCLASS =>
                    if (if classmap != null: 1 else: 0) != 0:
                                                (c = 0)
                        while (if c < 32: 1 else: 0) != 0:
re.start_bitmap[c] = re.start_bitmap[c] | classmap[c]                            (c = c + 1)
