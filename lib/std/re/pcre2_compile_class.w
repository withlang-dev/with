...
fn add_to_class(options: c_uint, xoptions: c_uint, cb: *mut compile_block_8, start: c_uint, end: c_uint):
    var classbits: *mut u8 = null // init: untranslatable
    var c: c_uint = 0 // init: untranslatable
    var byte_start: c_uint = 0 // init: untranslatable
    var byte_end: c_uint = 0 // init: untranslatable
    var classbits_end: c_uint = 0 // init: untranslatable
    xoptions
...
(classbits[c] = 255)        (c = c + 1)

    byte_start = byte_start << 3
    byte_end = byte_end << 3

fn add_list_to_class(options: c_uint, xoptions: c_uint, cb: *mut compile_block_8, p: *const c_uint):
    var p = p
...
fn add_not_list_to_class(options: c_uint, xoptions: c_uint, cb: *mut compile_block_8, p: *const c_uint):
    var p = p
...
fn fold_negation(pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong, preserve_classbits: c_int):
    ...
    if (not preserve_classbits) != 0:
        var i: c_int = 0
        while (if i < 8: 1 else: 0) != 0:
(pop_info.bits.classwords[i] = (0 - pop_info.bits.classwords[i] - 1))            (i = i + 1)
...
fn fold_binary(op: c_int, lhs_op_info: *mut eclass_op_info, rhs_op_info: *mut eclass_op_info, lengthptr: *mut c_ulong):
    match op
        1 =>
            var i: c_int = 0
            while (if i < 8: 1 else: 0) != 0:
lhs_op_info.bits.classwords[i] = lhs_op_info.bits.classwords[i] & rhs_op_info.bits.classwords[i]                (i = i + 1)
...
fn compile_eclass_nested(context: *mut eclass_context, negated: c_int, pptr: *mut *mut c_uint, pcode: *mut *mut u8, pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong) -> c_int:
    var negated = negated
    var ptr: *mut c_uint = null // init: untranslatable
    if (if unsafe: *(ptr = ptr + 1) == ((2148401152 | 1)): 1 else: 0) != 0:
        (negated = (not negated))
                ...
                match meta
                    2148270080 =>
                        ...
                        if (if ((if meta == 2148204544: 1 else: 0)) == negated: 1 else: 0) != 0:
                            (unsafe: *(code = code + 1) = (pop_info.op_single_type = 6))
                            with_memset(pop_info.bits.classbits as *i8, 255, 32 as i64)
                        else:
                            (unsafe: *(code = code + 1) = (pop_info.op_single_type = 7))
                            with_memset(pop_info.bits.classbits as *i8, 0, 32 as i64)

                    2148139008 =>
                        (ptr = ptr + 1)
                        (prev_ptr = ptr)
                        (ptr = _pcre2_compile_class_not_nested_8(context.options, context.xoptions, ptr, &code, (if ((if meta != 2148401152: 1 else: 0)) == negated: 1 else: 0), &context.needs_bitmap, context.errorcodeptr, context.cb, lengthptr))
                        if (if ptr == null: 1 else: 0) != 0:
                            return 0
                        ...
                        (extra_length = (if ((if lengthptr != null: 1 else: 0)) != 0: (unsafe: *lengthptr -% prev_length) else: 0))
                        if (if unsafe: *code_start == OP_ALLANY: 1 else: 0) != 0:
                            (pop_info.length = 1)
                            (unsafe: *code_start = (pop_info.op_single_type = 6))
                            with_memset(pop_info.bits.classbits as *i8, 255, 32 as i64)
                        else:
                            if (if (if unsafe: *code_start == OP_CLASS: 1 else: 0) != 0 or (if unsafe: *code_start == OP_NCLASS: 1 else: 0) != 0: 1 else: 0) != 0:
                                (pop_info.length = 1)
                                (unsafe: *code_start = (pop_info.op_single_type = (if ((if unsafe: *code_start == OP_CLASS: 1 else: 0)) != 0: 7 else: 6)))
                                with_memcpy(pop_info.bits.classbits as *i8, (code_start + (1 as isize as usize)) as *i8, 32 as i64)
                                if (if lengthptr != null: 1 else: 0) != 0:
                                ...
                                if (if (not context.needs_bitmap) != 0 and (if unsafe: *code_start == 7: 1 else: 0) != 0: 1 else: 0) != 0:
                                    var classwords: *mut c_uint = null // init: untranslatable
                                    var i: c_int = 0
                                    while (if i < 8: 1 else: 0) != 0:
if (if classwords[i] != 0: 1 else: 0) != 0:
                                ...
                                (unsafe: *code_start = (pop_info.op_single_type = 5))
                                with_memcpy(pop_info.bits.classbits as *i8, context.cb.classbits.classbits as *i8, 32 as i64)
...
if (if classwords[i] != 0: 1 else: 0) != 0:
                ...
                (pop_info.code_start = (if ((if lengthptr == null: 1 else: 0)) != 0: code_start else: null))
                if (if lengthptr != null: 1 else: 0) != 0:
...
fn compile_class_juxtaposition(context: *mut eclass_context, negated: c_int, pptr: *mut *mut c_uint, pcode: *mut *mut u8, pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong) -> c_int:
    var ptr: *mut c_uint = null // init: untranslatable
    var code: *mut u8 = null // init: untranslatable
    if (not compile_class_operand(context, negated, &ptr, &code, pop_info, lengthptr)) != 0:
        return 0

    while (if (if unsafe: *ptr != 2148335616: 1 else: 0) != 0 and (not ((if (if unsafe: *ptr >= 2151940096: 1 else: 0) != 0 and (if unsafe: *ptr <= 2152202240: 1 else: 0) != 0: 1 else: 0))) != 0: 1 else: 0) != 0:
        var op: c_uint = 0 // init: untranslatable
        var rhs_negated: c_int = 0 // init: untranslatable
        var rhs_op_info = 0 // init: untranslatable (eclass_op_info)
        if negated != 0:
            (op = 1)
            (rhs_negated = 1)
        else:
        ...
        if (not compile_class_operand(context, rhs_negated, &ptr, &code, &rhs_op_info, lengthptr)) != 0:
            return 0
...
fn compile_class_unary(context: *mut eclass_context, negated: c_int, pptr: *mut *mut c_uint, pcode: *mut *mut u8, pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong) -> c_int:
    var negated = negated
    var ptr: *mut c_uint = null // init: untranslatable
...
fn compile_class_binary_tight(context: *mut eclass_context, negated: c_int, pptr: *mut *mut c_uint, pcode: *mut *mut u8, pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong) -> c_int:
    var ptr: *mut c_uint = null // init: untranslatable
    var code: *mut u8 = null // init: untranslatable
    if (not compile_class_unary(context, negated, &ptr, &code, pop_info, lengthptr)) != 0:
        return 0
    ...
    while (if unsafe: *ptr == 2151940096: 1 else: 0) != 0:
        ...
        (ptr = ptr + 1)
        if (not compile_class_unary(context, rhs_negated, &ptr, &code, &rhs_op_info, lengthptr)) != 0:
            return 0
...
fn compile_class_binary_loose(context: *mut eclass_context, negated: c_int, pptr: *mut *mut c_uint, pcode: *mut *mut u8, pop_info: *mut eclass_op_info, lengthptr: *mut c_ulong) -> c_int:
    var ptr: *mut c_uint = null // init: untranslatable
    var code: *mut u8 = null // init: untranslatable
    if (not compile_class_binary_tight(context, negated, &ptr, &code, pop_info, lengthptr)) != 0:
        return 0

    while (if (if unsafe: *ptr >= 2152005632: 1 else: 0) != 0 and (if unsafe: *ptr <= 2152136704: 1 else: 0) != 0: 1 else: 0) != 0:
        var op: c_uint = 0 // init: untranslatable
        var op_neg: c_int = 0 // init: untranslatable
        var rhs_negated: c_int = 0 // init: untranslatable
        var rhs_op_info = 0 // init: untranslatable (eclass_op_info)
        if negated != 0:
        ...
        (ptr = ptr + 1)
        if (not compile_class_binary_tight(context, rhs_negated, &ptr, &code, &rhs_op_info, lengthptr)) != 0:
            return 0
