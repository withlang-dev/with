...
fn pcre2_serialize_encode_8(codes: *mut *const pcre2_real_code_8, number_of_codes: c_int, serialized_bytes: *mut *mut u8, serialized_size: *mut c_ulong, gcontext: *mut pcre2_real_general_context_8) -> c_int:
    ...
    with_memcpy(bytes, memctl, sizeof[pcre2_memctl]())
    ...
    with_memcpy(dst_bytes, tables, 1088)
    (i = 0)
    while (if i < number_of_codes: 1 else: 0) != 0:
        with_memcpy(dst_bytes, (re as *const i8), re.blocksize)
        with_memset((dst_bytes + 0), 0, sizeof[pcre2_memctl]())
    ...
    while true:
        match __pc
            0 =>
                ...
                with_memcpy(tables, src_bytes, 1088)
                (i = 0)
                while (if i < number_of_codes: 1 else: 0) != 0:
                    var blocksize: c_ulong = 0 // init: untranslatable
                    with_memcpy(&blocksize, (src_bytes + 72), sizeof[c_ulong]())
