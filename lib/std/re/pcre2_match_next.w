...
pub unsafe fn pcre2_next_match_8(__param_match_data: *mut pcre2_real_match_data_8, __param_pstart_offset: *mut c_ulong, __param_poptions: *mut c_uint) -> c_int {
    ...
    var __local_ovector: *mut c_ulong = ((&raw const (*__param_match_data).ovector[0] as *mut c_ulong))
    ...
    if ((if (__local_ovector[0]) != __local_start_offset: 1 else: 0) != 0) {
        (__ci_expr_logic_0 = (if (if (__local_ovector[1]) == __local_start_offset: 1 else: 0) != 0: 1 else: 0))
    }
    ...
    if (__ci_expr_logic_0 != 0) {
        ...
        ((*__param_pstart_offset) = ((do_bumpalong(__param_match_data, ((__local_ovector[1]) as c_ulong)) as c_ulong)))

        ((*__param_poptions) = ((0 as c_uint)))
    ...
    if ((if (__local_ovector[0]) == (__local_ovector[1]): 1 else: 0) != 0) {
        if ((if (__local_ovector[0]) >= (*__param_match_data).subject_length: 1 else: 0) != 0) {
            return 0
        }

        ((*__param_pstart_offset) = (((__local_ovector[1]) as c_ulong)))

        ((*__param_poptions) = ((8 as c_uint)))
...
unsafe fn do_bumpalong(__param_match_data: *mut pcre2_real_match_data_8, __param_offset: c_ulong) -> c_ulong {
    ...
    var __local_utf: c_int = (((if (((*(*__param_match_data).code).overall_options as c_uint) & (524288 as c_uint)) != 0: 1 else: 0) as c_int))

    var __ci_expr_logic_1: c_int = 0

    var __ci_expr_logic_0: c_int = 0

    if ((if (__local_subject[__param_offset]) == 13: 1 else: 0) != 0) {
        (__ci_expr_logic_0 = (if (if ((__param_offset as c_ulong) +% (1 as c_ulong)) < __local_subject_length: 1 else: 0) != 0: 1 else: 0))
    }

    if (__ci_expr_logic_0 != 0) {
        (__ci_expr_logic_1 = (if (if (__local_subject[((__param_offset as c_ulong) +% (1 as c_ulong))]) == 10: 1 else: 0) != 0: 1 else: 0))
    }
    ...
    if (__local_utf != 0) {
        ...
        while true {
            ...
            if ((if __local_next < __local_subject_end: 1 else: 0) != 0) {
                (__ci_expr_logic_2 = (if (if ((((*__local_next) as c_int) as c_uint) & (192 as c_uint)) == 128: 1 else: 0) != 0: 1 else: 0))
            }
