// Migrated from PCRE2 — pcre2_error.c
use std.re.defs
...
fn pcre2_get_error_message_8(p0: c_int, p1: *mut u8, p2: c_ulong) -> c_int:
...
if (if enumber < 0: 1 else: 0) != 0:
    ...
    while (if unsafe: *message != 0: 1 else: 0) != 0:
        ...
        (buffer[i] = unsafe: *{ let __tmp = message; message = message + 1; __tmp })
        { let __tmp = i; i = i + 1; __tmp }

    (buffer[i] = 0)
    return (if rc != 0: rc else: (i as c_int))
