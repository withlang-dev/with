...
fn base32_validate(text: &str) -> Result[i64, DecodeError]:
    ...
    if padding > 0:
        ...
        let value = base32_value(text[offset])
...
pub fn base32_encode(data: []u8) -> str:
    ...
    while offset < data.len():
        ...
        let v6 = ((b3 & 3) << 3) | (b4 >> 5)
        let v7 = b4 & 31
        out.push_byte(BASE32_ALPHABET[v0] as u8)
        out.push_byte(BASE32_ALPHABET[v1] as u8)
        if symbols > 2: out.push_byte(BASE32_ALPHABET[v2] as u8) else: out.push_byte(61 as u8)
        if symbols > 3: out.push_byte(BASE32_ALPHABET[v3] as u8) else: out.push_byte(61 as u8)
        if symbols > 4: out.push_byte(BASE32_ALPHABET[v4] as u8) else: out.push_byte(61 as u8)
        if symbols > 5: out.push_byte(BASE32_ALPHABET[v5] as u8) else: out.push_byte(61 as u8)
        if symbols > 6: out.push_byte(BASE32_ALPHABET[v6] as u8) else: out.push_byte(61 as u8)
        if symbols > 7: out.push_byte(BASE32_ALPHABET[v7] as u8) else: out.push_byte(61 as u8)
        offset = offset + count
    out.to_str()
...
pub fn base32_decode(text: &str) -> Result[Vec[u8], DecodeError]:
    ...
    while offset < text.len():
        ...
        let v0 = base32_value(text[offset])
        let v1 = base32_value(text[offset + 1])
        let v2 = if symbols > 2: base32_value(text[offset + 2]) else: 0
        let v3 = if symbols > 3: base32_value(text[offset + 3]) else: 0
        let v4 = if symbols > 4: base32_value(text[offset + 4]) else: 0
        let v5 = if symbols > 5: base32_value(text[offset + 5]) else: 0
        let v6 = if symbols > 6: base32_value(text[offset + 6]) else: 0
        let v7 = if symbols > 7: base32_value(text[offset + 7]) else: 0
