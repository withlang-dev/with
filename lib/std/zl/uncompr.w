// Migrated from C
use std.zlib.defs
use std.zlib.zutil
use std.zlib.deflate
use std.zlib.inflate
use std.zlib.infback
use std.zlib.compress
use std.zlib.gzlib
use std.zlib.gzwrite
use std.zlib.gzread
use std.zlib.gzclose
use std.zlib.adler32
use std.zlib.crc32

pub unsafe fn uncompress(__param_dest: *mut u8, __param_destLen: *mut c_ulong, __param_source: *const u8, __param_sourceLen: c_ulong) -> c_int {
    var __local_used: c_ulong = __param_sourceLen

    return uncompress2(__param_dest, __param_destLen, __param_source, (&raw mut __local_used as *mut c_ulong))

}

pub unsafe fn uncompress_z(__param_dest: *mut u8, __param_destLen: *mut c_ulong, __param_source: *const u8, __param_sourceLen: c_ulong) -> c_int {
    var __local_used: c_ulong = __param_sourceLen

    return uncompress2_z(__param_dest, __param_destLen, __param_source, (&raw mut __local_used as *mut c_ulong))

}

pub unsafe fn uncompress2(__param_dest: *mut u8, __param_destLen: *mut c_ulong, __param_source: *const u8, __param_sourceLen: *mut c_ulong) -> c_int {
    var __local_ret: c_int

    var __local_got: c_ulong = (unsafe *__param_destLen)

    var __local_used: c_ulong = (unsafe *__param_sourceLen)


    (__local_ret = ((uncompress2_z(__param_dest, (&raw mut __local_got as *mut c_ulong), __param_source, (&raw mut __local_used as *mut c_ulong)) as c_int)))

    ((unsafe *__param_sourceLen) = __local_used)

    ((unsafe *__param_destLen) = __local_got)

    return __local_ret

}

pub unsafe fn uncompress2_z(__param_dest: *mut u8, __param_destLen: *mut c_ulong, __param_source: *const u8, __param_sourceLen: *mut c_ulong) -> c_int {
    var __local_dest = __param_dest
    var __local_stream: z_stream_s

    var __local_err: c_int

    var __local_max: c_uint = ((-1 as c_uint))

    var __local_len: c_ulong

    var __local_left: c_ulong


    var __ci_expr_logic_4: c_int

    var __ci_expr_logic_2: c_int

    var __ci_expr_logic_1: c_int

    if ((if __param_sourceLen == null: 1 else: 0) != 0) {
        (__ci_expr_logic_1 = (if true: 1 else: 0))
    } else {
        var __ci_expr_logic_0: c_int = 0

        if ((if (unsafe *__param_sourceLen) > 0: 1 else: 0) != 0) {
            (__ci_expr_logic_0 = (if (if __param_source == null: 1 else: 0) != 0: 1 else: 0))
        }

        (__ci_expr_logic_1 = (if __ci_expr_logic_0 != 0: 1 else: 0))

    }

    if (__ci_expr_logic_1 != 0) {
        (__ci_expr_logic_2 = (if true: 1 else: 0))
    } else {
        (__ci_expr_logic_2 = (if (if __param_destLen == null: 1 else: 0) != 0: 1 else: 0))
    }

    if (__ci_expr_logic_2 != 0) {
        (__ci_expr_logic_4 = (if true: 1 else: 0))
    } else {
        var __ci_expr_logic_3: c_int = 0

        if ((if (unsafe *__param_destLen) > 0: 1 else: 0) != 0) {
            (__ci_expr_logic_3 = (if (if __local_dest == null: 1 else: 0) != 0: 1 else: 0))
        }

        (__ci_expr_logic_4 = (if __ci_expr_logic_3 != 0: 1 else: 0))

    }

    if (__ci_expr_logic_4 != 0) {
        return -2
    }


    (__local_len = (unsafe *__param_sourceLen))

    (__local_left = (unsafe *__param_destLen))

    var __ci_expr_logic_5: c_int = 0

    if ((if __local_left == 0: 1 else: 0) != 0) {
        (__ci_expr_logic_5 = (if (if __local_dest == 0: 1 else: 0) != 0: 1 else: 0))
    }

    if (__ci_expr_logic_5 != 0) {
        (__local_dest = ((((&raw const (unsafe *(&raw const __local_stream as *const z_stream_s)).reserved as *const c_ulong) as *mut c_ulong) as *mut u8)))
    }


    (__local_stream.next_in = ((__param_source as *mut u8)))

    (__local_stream.avail_in = ((0 as c_uint)))

    (__local_stream.zalloc = ((0 as unsafe extern "C" fn(*mut c_void, c_uint, c_uint) -> *mut c_void)))

    (__local_stream.zfree = ((0 as unsafe extern "C" fn(*mut c_void, *mut c_void) -> Unit)))

    (__local_stream.opaque_ = ((0 as *mut c_void)))

    (__local_err = ((inflateInit_((&raw mut __local_stream as *mut z_stream_s), c"1.3.2".ptr, (sizeof[z_stream_s]() as c_int)) as c_int)))

    if ((if __local_err != 0: 1 else: 0) != 0) {
        return __local_err
    }

    (__local_stream.next_out = __local_dest)

    (__local_stream.avail_out = ((0 as c_uint)))

    loop {
        if ((if (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_out == 0: 1 else: 0) != 0) {
            var __ci_expr_ternary_6: c_uint = 0

            if ((if __local_left > ((4294967295 as c_ulong)): 1 else: 0) != 0) {
                (__ci_expr_ternary_6 = ((4294967295 as c_uint)))
            } else {
                (__ci_expr_ternary_6 = ((__local_left as c_uint)))
            }

            (__local_stream.avail_out = __ci_expr_ternary_6)


            (__local_left = (__local_left -% (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_out))

        }

        if ((if (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_in == 0: 1 else: 0) != 0) {
            var __ci_expr_ternary_7: c_uint = 0

            if ((if __local_len > ((4294967295 as c_ulong)): 1 else: 0) != 0) {
                (__ci_expr_ternary_7 = ((4294967295 as c_uint)))
            } else {
                (__ci_expr_ternary_7 = ((__local_len as c_uint)))
            }

            (__local_stream.avail_in = __ci_expr_ternary_7)


            (__local_len = (__local_len -% (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_in))

        }

        (__local_err = ((inflate((&raw mut __local_stream as *mut z_stream_s), (0 as c_int)) as c_int)))

        if not (((if __local_err == 0: 1 else: 0) != 0)) {
            break
        }
    }

    (__local_len = (__local_len +% (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_in))

    (__local_left = (__local_left +% (unsafe *(&raw const __local_stream as *const z_stream_s)).avail_out))

    ((unsafe *__param_sourceLen) = ((unsafe *__param_sourceLen) -% __local_len))

    ((unsafe *__param_destLen) = ((unsafe *__param_destLen) -% __local_left))

    inflateEnd((&raw mut __local_stream as *mut z_stream_s))

    var __ci_expr_ternary_11: c_int = 0

    if ((if __local_err == 1: 1 else: 0) != 0) {
        (__ci_expr_ternary_11 = ((0 as c_int)))
    } else {
        var __ci_expr_ternary_10: c_int = 0

        if ((if __local_err == 2: 1 else: 0) != 0) {
            (__ci_expr_ternary_10 = ((-3 as c_int)))
        } else {
            var __ci_expr_ternary_9: c_int = 0

            var __ci_expr_logic_8: c_int = 0

            if ((if __local_err == -5: 1 else: 0) != 0) {
                (__ci_expr_logic_8 = (if (if __local_len == 0: 1 else: 0) != 0: 1 else: 0))
            }

            if (__ci_expr_logic_8 != 0) {
                (__ci_expr_ternary_9 = ((-3 as c_int)))
            } else {
                (__ci_expr_ternary_9 = __local_err)
            }

            (__ci_expr_ternary_10 = __ci_expr_ternary_9)

        }

        (__ci_expr_ternary_11 = __ci_expr_ternary_10)

    }

    return __ci_expr_ternary_11


}
